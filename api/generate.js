...
const buildPrompt = (type, payload) => {
  if (type === "content") {
    ...
    return `크잉에듀 원장님이 네이버블로그 지역 노출에 활용할 수 있는 블로그 포스팅만 작성해줘. 인스타그램 캡션은 작성하지 마.
...
- 본문 초안 안에는 "${region}"를 자연스럽게 정확히 5번 포함
- 본문은 공백 제외 약 1500자 분량으로 작성
- 본문에는 읽기 쉬운 소제목을 3~4개 포함
- 소제목에는 #, ##, ### 같은 마크다운 기호를 쓰지 말고 일반 문장형 소제목으로 작성
- 해시태그는 정확히 5개만 작성
- 해시태그는 줄바꿈 없이 한 줄로 작성
- 지역 키워드를 억지로 반복하지 말고 문장 안에 자연스럽게 녹여줘

도입부 강화 규칙
- 도입부 첫 3~4문장은 약하게 소개하지 말고 학부모가 바로 공감할 문제로 시작
- 예: "아이가 영어를 외우기만 하고 말하지 못한다", "파닉스를 배워도 읽기로 이어지지 않는다", "수업은 다니는데 흥미가 오래가지 않는다" 같은 고민을 자연스럽게 건드려줘
- 도입부 안에서 지역 키워드, 학부모 고민, 교구 수업의 차별점을 모두 보여줘
- 첫 문단 끝에는 글을 계속 읽고 싶게 만드는 문장을 넣어줘
        ...
        input: [
          {
            role: "system",
            content:
              "너는 크잉에듀 Creative English의 네이버블로그 SEO와 지역 마케팅 콘텐츠 전문가다. 블로그와 릴스 목적을 분리해서 원장님이 복사해 바로 쓸 수 있는 한국어 콘텐츠를 만든다."
          },
          {
            role: "user",
            content: prompt
